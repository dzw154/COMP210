...
public class LinkedList<T> {
    ...
    public boolean contains(Object element) { // Checks if the linked list contains the given element
        Node<T> current = head;
        while(current != null) {
    ...
    public T[] toArray() { // Converts the linked list to an array
    ...
    public boolean remove(Object element) { // Removes the current object from the linked list
        Node<T> current = head;
        if(isEmpty()) {
            return false;
        }
    ...
    public T get(int index) {  // Gets the object at an index
        validIndex(index);
        Node<T> current = head;
        int i = 0;
        while (i < index) {
    ...
    public T set(int index, Object element) { // Sets the value of a node in the list at the given index to the element
        validIndex(index);
        Node<T> current = head;
        T prevValue = null;
        int i = 0;
    ...
    public void add(int index, Object element) { // Adds an object at a given element to the linked list
        if(index > size) {
            validIndex(index);
        }
        Node<T> current = head;
    ...
    public int indexOf(Object element) { // Gets index of an object in the linked list
        Node<T> current = head;
        int index = 0;
        while(current != null) {
    ...
    public int lastIndexOf(Object element) { // Gets the last index an object occurs at in the linked list
        Node<T> current = head;
        int index = -1;
        int i = 0;
        while(current != null) {
    ...
    public void validIndex(int i) { // Checks if index is in range
        if(i < 0 || i >= size) {
            throw new IndexOutOfBoundsException("Invalid index");
        }
    }
    public Node<T> getHead() { // Gets head
        return head;
    }
